...
metaRouter.get("/meta", asyncHandler(async (_req, res) => { res.json({ data: { categories: SKILL_CATEGORIES, cities: CITIES.map(({ city, canton }) => ({ city, canton })), languages: LANGUAGE_SUGGESTIONS, languageSuggestions: LANGUAGE_SUGGESTIONS, durations: DURATIONS, modes: LISTING_MODES, levels: LEVELS } }); }));
